    ...
    public static class JiXianTiaoZhanActivity
    {
        ...
        public static async Task<JsonResult> GetRankList(JiXianTiaoZhanController mJiXianTiaoZhanController)
        {
            ...
            try
            {
                ...
                var mSameActivityList = await mDbContex.JiXianTiaoZhanInfoList.AsNoTracking().Where((x) => x.nActivityId == nActivityId).Join(
                    mDbContex.t_user,
                ...
                var mSortQueue = mSameActivityList.OrderByDescending((x) => x.entryPoint.nTime)
                    .ThenByDescending((x) => x.entryPoint.nScore).ThenBy((x) => x.entryPoint.nUserId).Take(50);

                List<ResultPlayerRankInfo> mPlayerRankInfoList = new List<ResultPlayerRankInfo>();
                uint nRankIndex = 1;
                foreach (var v in mSortQueue)
                {
                    ResultPlayerRankInfo mInfo = GetResultPlayerInfo(v.entryPoint);
                    mInfo.nRank = nRankIndex++;
                    mInfo.strName = v.entry.nick;
                    mInfo.iconUrl = v.entry.icon;
                    mPlayerRankInfoList.Add(mInfo);
                }

                var mDbCombineInfo = mSameActivityList.FirstOrDefault((x) => x.entryPoint.nUserId == userId);
                if (mDbCombineInfo != null)
                {
                    int nCount = mSameActivityList.Where((x) =>
                            (x.entryPoint.nTime > mDbCombineInfo.entryPoint.nTime ||
                            (x.entryPoint.nTime == mDbCombineInfo.entryPoint.nTime && x.entryPoint.nScore > mDbCombineInfo.entryPoint.nScore) ||
                            (x.entryPoint.nTime == mDbCombineInfo.entryPoint.nTime && x.entryPoint.nScore == mDbCombineInfo.entryPoint.nScore &&
                                x.entryPoint.nUserId.CompareTo(mDbCombineInfo.entryPoint.nUserId) < 0))).Count();

                    mDbCombineInfo.entryPoint.nRank = (uint)nCount + 1;
                }
                ...
                if (mDbCombineInfo != null)
                {
                    var mRankInfo = GetResultPlayerInfo(mDbCombineInfo.entryPoint);
                    mRankInfo.strName = mDbCombineInfo.entry.nick;
                    mRankInfo.iconUrl = mDbCombineInfo.entry.icon;
